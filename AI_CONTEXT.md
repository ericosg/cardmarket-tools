...
- **Expected Value (EV) Calculator with Scryfall integration**
...
├── ev/
│   ├── types.ts         # EV TypeScript interfaces
...
│   └── ev-calculator.ts        # Main EV calculation coordinator
├── commands/
│   ├── types.ts         # Shared TypeScript interfaces
│   ├── search.ts        # Search command logic (export + API + EV)
...
- `data/` - Export data files directory (gitignored)
  ...
  - `scryfall_cards.json` - ~26MB Scryfall card data
...
### Expected Value (EV) System (src/ev/)

**Architecture Overview:**
The EV system calculates expected value of sealed booster products by combining:
1. Cardmarket sealed product prices (from export data)
2. Scryfall card prices in EUR (from bulk data API)
3. Exact booster pack composition rules (from collation database)
4. Auto-mapping between Cardmarket and Scryfall set identifiers

**Scryfall Downloader (scryfall-downloader.ts):**
- **Purpose:** Download and process Scryfall bulk card data
- **Data Source:** https://api.scryfall.com/bulk-data (Default Cards type)
- **Update Frequency:** Weekly (configurable: daily/weekly/manual)
- **Data Size:** ~26MB processed, 98,349+ cards
- **Processing:**
  - Filters for paper-only cards
  - Excludes tokens and non-collectible cards
  - Extracts: id, name, set, set_name, rarity, eur, eur_foil, collector_number, released_at
  - Normalizes rarity values
- **Storage:** `data/scryfall_cards.json` + `data/scryfall_metadata.json`
- **Key Methods:**
  - `downloadBulkData(force)` - Download if >7 days old or forced
  - `needsUpdate()` - Check age threshold
  - `getDataStatus()` - Get download status and metadata
  - `loadProcessedCards()` - Load from disk

**Scryfall Loader (scryfall-loader.ts):**
- **Purpose:** Load and index Scryfall data for fast lookups
- **Indexing Strategy:** Three-tier O(1) indexing
  - By card ID: `Map<string, ProcessedCard>`
  - By set code: `Map<string, ProcessedCard[]>`
  - By set + rarity: Nested maps for rarity filtering
- **Key Methods:**
  - `load()` - Load and index all cards
  - `getCard(id)` - Get single card by ID
  - `getSetCards(setCode)` - Get all cards in a set
  - `getSetCardsByRarity(setCode, rarity)` - Rarity-filtered cards
  - `getAllSetCodes()` - Get list of all set codes
  - `getSetStats(setCode, bulkThreshold)` - Calculate set statistics
- **Performance:** O(1) lookups, handles 98k+ cards efficiently

**Expansion Mapper (expansion-mapper.ts):**
- **Purpose:** Map Cardmarket expansion IDs to Scryfall set codes
- **Mapping Strategy:**
  1. Auto-mapping via card name matching (singles)
  2. Auto-mapping via set name matching (nonsingles/sealed products)
  3. Manual overrides for edge cases
- **Confidence Scoring:** 0.5-1.0 based on match percentage
- **Storage:** `data/expansion_mapping.json`
  - `manualOverrides` - Human-specified mappings (priority)
  - `autoGenerated` - Algorithm-generated mappings
- **Auto-Mapping Algorithm:**
  - For singles: Sample 20 cards per expansion, find Scryfall set with >50% card name matches
  - For nonsingles: Extract set name from product name, match to Scryfall set_name
  - Regex pattern for sealed products: `^(.+?)(?:\s+(?:Play|Draft|Set|Collector|Bundle|Booster|Box|...))`
  - Normalizes names by removing non-alphanumeric, "magic", "thegathering"
- **Current Coverage:** 374+ expansions mapped
- **Key Methods:**
  - `initialize(scryfallLoader)` - Generate auto-mappings
  - `getSetCode(expansionId)` - Cardmarket ID → Scryfall code
  - `getExpansionId(setCode)` - Reverse lookup
  - `addManualMapping()` - Add override
  - `saveMappings()` - Persist to file

**Collation Engine (collation-engine.ts):**
- **Purpose:** Calculate EV using exact booster pack composition rules
- **Collation Rules:** `data/booster_collation.json`
  - Default rules for: Play Booster, Draft Booster, Set Booster, Collector Booster
  - Set-specific overrides for special sets (MH3, 2XM, etc.)
- **Booster Structure:**
  ```typescript
  {
    type: 'playBooster',
...
- **EV Calculation Formula:**
  - Per slot: `average(card prices) × count × weight`
  - Rare/Mythic: `(mythic_avg × 1/8 + rare_avg × 7/8) × count`
  - Bulk threshold: Cards < €1.00 excluded
  - Foil adjustment: Average foil premium × foil rate
- **Box EV:** Pack EV × pack count (usually 36)
- **Key Methods:**
  - `load()` - Load collation rules from JSON
  - `getBoosterConfig(setCode, boosterType)` - Get pack composition
  - `calculatePackEV(...)` - Calculate single pack EV
  - `calculateBoxEV(...)` - Calculate box EV with top cards
- **Set-Specific Rules:** Currently 8 sets with custom configurations

**EV Calculator (ev-calculator.ts):**
- **Purpose:** Main coordinator for EV calculations
- **Initialization:** Loads Scryfall data, expansion mapper, collation engine
- **Workflow:**
  1. Map Cardmarket expansion ID → Scryfall set code
  2. Load all cards for that set
  3. Determine booster type from product name
  4. Get pack count from BoosterCountLookup utility
  5. Calculate pack EV using collation rules
  6. Calculate box EV (pack EV × count)
  7. Compute EV ratio and difference
- **Result Structure:**
  ```typescript
  {
    boxEV: 495.35,              // Total box expected value
    packEV: 13.76,              // Per-pack expected value
    evRatio: 3.35,              // Box EV / sealed price
    evDifference: 347.63,       // Box EV - sealed price
    confidence: 0.92,           // Data quality score
    breakdown: {
      mythicContribution: 8.45,
      rareContribution: 4.12,
      uncommonContribution: 0.89,
      commonContribution: 0.15,
      foilContribution: 0.15
    },
    topCards: [...],            // Top 20 value cards
    variance: { min, median, max }
  }
  ```
- **Booster Type Detection:** Inferred from product name keywords
- **Key Methods:**
  - `initialize()` - Load all dependencies
  - `calculateEV(productId, productName, ...)` - Calculate single product
  - `calculateSetEVs(expansionId)` - Calculate all products in set
  - `getTopValueCards(...)` - Get high-value cards

**Integration with Search:**
- **CLI Flag:** `--show-ev` on search command
- **Display:** Adds 3 columns: Box EV, EV Ratio, Status
- **Color Coding:**
  - Green "✓ Pos": Ratio ≥ 1.2 (positive EV)
  - Yellow "~": Ratio ≥ 1.0 (break even)
  - Red "✗ Neg": Ratio < 1.0 (negative EV)
- **Example Output:**
  ```
  Bloomburrow Play Booster Box | €147.72 avg | €4.10 per booster | Box EV: €495.35 | Ratio: 3.35x | ✓ Pos
  ```
- **Performance:** Initialization ~5s (loads 98k cards + generates mappings), calculations ~100ms per product
...
- **Schema:**
  ```json
  {
    ...
    "ev": {
      "enabled": true,
      "autoUpdate": true,
      "updateFrequency": "weekly (or: daily, manual)",
